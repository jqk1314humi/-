...
class VikaCloudStorage {
    ...
    async useActivationCode(code, deviceInfo) {
        ...
        try {
            ...
            // 1. 先检查激活码是否已经在使用记录表中
            const usedInUsageTable = await this.checkUsageTable(code);
            if (usedInUsageTable) {
                throw new Error('激活码已被使用');
            }

            // 2. 使用维格表API将激活码写入使用记录表
            console.log('🔄 将激活码写入使用记录维格表:', code);
            await this.writeToUsageTable(code, deviceInfo);
            ...
            console.log('🔄 更新审核维格表中的激活码状态:', code, updateFields);
